...
def get_available_restaurants(origin, destination, rest_delivery_distance):
    ...
    for id, values in enumerate(directions_result['rows'][0]['elements']):
        ...
        if distance_meters <= rest_delivery_distance[directions_result['destination_addresses'][id]]*1000:
            valid_addresses.append(directions_result['destination_addresses'][id])
...
def insert_address(address):
    ...
    cur.execute(f"SELECT address, delivery_distance FROM restaurant WHERE voivodeship = '{voivodeship}';")
    ...
    cur.execute(f"INSERT INTO address (address_id, address_name, voivodeship) VALUES('{new_address_id}', '{address}', '{voivodeship}')")
    ...
    try:
        ...
        passed = False
    if passed:
        rest_delivery_distance = {tuple_of_data[0]:tuple_of_data[1] for tuple_of_data in fetched_data}
        ...
        available_restaurants = get_available_restaurants(origin=address, destination=dest_addresses, rest_delivery_distance=rest_delivery_distance)
        for restaurant in available_restaurants:
            cur.execute(f"SELECT restaurant_id FROM restaurant WHERE address = '{restaurant}';")
            restaurant_id = cur.fetchall()[0][0]

            cur.execute(f"INSERT INTO restaurant_address (restaurant_id, address_id) VALUES ('{restaurant_id}', '{new_address_id}')")
